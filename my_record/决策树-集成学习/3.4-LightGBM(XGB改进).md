...
1. XGBoost 使用预排序后**需要记录特征值及其对应样本的统计值的索引**，而 LightGBM 使用了**直方图算法将特征值转变为 bin 值，且不需要记录特征到样本的索引**，更高（效率）更快（速度）更低（内存占用）更泛化；
2. LightGBM 在训练过程中采用**互斥特征捆绑算法减少了特征数量**，降低了内存消耗。
3. LightGBM 在训练过程中采用**单边梯度算法过滤掉梯度小的样本**，减少计算；
4. LightGBM 采用了**基于 Leaf-wise 算法的增长策略构建树，减少计算量**；
5. LightGBM 采用**优化后的特征并行、数据并行方法加速计算**，当数据量非常大的时候还可以采用投票并行的策略；
6. LightGBM 对**缓存也进行了优化，增加了 Cache hit 的命中率**。
