...
# tf.train.shuffle_batch组合样例提供给神经网络输入层  不需要feed_dict?????????
# min_after_dequeue出队时队列中元素的最少个数（队列元素太少shuffle没用）
# capacity队列大小（太大占用内存，太小容易因为没有数据而block）
#  batch_size一个batch中样例的个数
...
# one_shot_iterator数据集所有参数必须已经确定
...
# 下载数据
mnist = input_data.read_data_sets("../../datasets/MNIST_data/", one_hot=True)
# print(mnist.train.num_examples, mnist.validation.images[2])
# batch_size = 100
...
# 计算交叉熵及其平均值#当问题只有一个正确答案时可以用这个函数加快交叉熵计算
cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=tf.argmax(y_, 1))
cross_entropy_mean = tf.reduce_mean(cross_entropy)
# 损失函数的计算
...
# 优化损失函数
train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss, global_step=global_step)
# 反向传播更新参数和更新每一个参数的滑动平均值，为了一次完成多个操作有以下两种机制
train_op = tf.group(train_step, ema_op)

# 计算正确率,tf.equal返回的是布尔值
...
#with tf.variable_scope("bar", reuse=True):
   # v = tf.get_variable("v", [1])  在bar空间中没有v
